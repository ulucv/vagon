...
generate "provider" {
  ...
  if_exists = "skip"  #added this conf since vpn needs assume role for a different provider
  contents  = <<EOF
provider "aws" {
...
remote_state {
  ...
  config = {
    encrypt        = true
    bucket         = "${local.project_name}-terraform-state-${local.aws_region}-${local.account_id}"
